namespace SubdivxSearch.Domain
{
    ...
    public class SubDivXManager
    {
        ...
        public IList<Sub> GetCandidateSubs(Video video, bool searchInComments, Dictionary<string, string> cache)
        {
            ...
            Debug.WriteLine("Release Group to match: {0}", video.ReleaseGroup);
            ...
            while (page > 0)
            {
                ...
                doc.LoadHtml(output);
                var subsHtmlNodes = doc.DocumentNode.Descendants("div").Where(div => div.GetAttributeValue("id", string.Empty).Equals("buscador_detalle")).ToList();

                if (!subsHtmlNodes.Any())
                {
                    page = -1;
                }
                else
